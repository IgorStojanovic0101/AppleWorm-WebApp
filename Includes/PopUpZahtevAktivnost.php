...
<div id="popup">
    ...
    <div id="popup_header">
        <label id="obavestenje">
            Obrada zahteva za aktivnost
        </label>
    </div>
    <div id="popup_body">
    <form method="post" action="<?php echo"$link"?>" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php echo "$zahtev->id"?>" /> <!--Mora jer se pravi obj na osnovu parametara koji se odavde salju-->
        <label>Naziv: </label> <input type="text" name="naziv" value="<?php echo"$zahtev->naziv";?>"/><br />
        <label>MET vrednost: </label> <input type="number" step="0.1" name="met_vrednost" value="<?php echo"$zahtev->met_vrednost";?>" /><br />
        <label>Slika: </label> <input class="slika" type="file" name="slika" /><br />

        <input type="submit" class="dugme" name="btnZahAktOK" value="Dodaj u bazu" />
        <input type="submit" class="dugme" name="btnZahAktObrisi" value="Obrisi" />
    </form>
    </div>
    <div id="popup_footer">
